...
Fuel is a lightweight, git-native task management system built for AI-assisted development workflows. It stores tasks in a JSONL file that's version-controlled alongside your code.

### Quick Start

```bash
# Add a task
php artisan fuel:add "Implement user authentication" --type=feature --priority=1
...
# Mark a task as done
php artisan fuel:done fuel-a7f3
```
...
| Command | Description |
|---------|-------------|
| `fuel:add` | Add a new task with title, type, priority, description, labels, and dependencies |
| `fuel:list` | List tasks with optional filters (status, type, priority, label) |
| `fuel:ready` | Show tasks that are ready to work on (no open blockers) |
| `fuel:done` | Mark a task as complete (supports partial ID matching) |
...
- `2` - Medium (nice-to-have features) *[default]*
- `3` - Low (polish, optimization)
- `4` - Backlog (future ideas)

### Dependencies

Tasks can block other tasks. When you create a task with `--blocked-by`, that task won't appear in `fuel:ready` until all blockers are closed:

```bash
# Create a blocking task
php artisan fuel:create "Design auth system" --type=task

# Create a task blocked by it
php artisan fuel:create "Implement auth" --type=feature --blocked-by=fuel-a7f3
```
...
Tasks are stored in `.ai/fuel.jsonl` - a git-friendly format where each line is a complete JSON object. The file is automatically kept sorted for minimal merge conflicts.
