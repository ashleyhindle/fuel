...
class InstallCommand extends Command
{
    ...
    protected function collectInstallationPreferences(): void
    {
        ...
        $this->enableFuel = $this->shouldEnableFuel();
    }
    ...
    protected function shouldEnableFuel(): bool
    {
        // Check if fuel is already initialized
        $service = app(TaskService::class);
        $alreadyInitialized = file_exists($service->getStoragePath());
        ...
        return confirm(
            label: 'Would you like to enable Fuel task tracking?',
            default: $this->config->getFuel(),
            hint: 'Fuel is a lightweight, git-native task management system for AI-assisted workflows',
        );
    }

    protected function initializeFuel(): void
    {
        $service = app(TaskService::class);

        if (file_exists($service->getStoragePath())) {
            $this->line("  {$this->greenTick} Fuel already initialized");

            return;
        }

        $service->initialize();
        $this->config->setFuel(true);

        $this->newLine();
        $this->line("  {$this->greenTick} Fuel task tracking initialized");
        $this->line('    Storage: '.str_replace(base_path().'/', '', $service->getStoragePath()));
        $this->line('    Get started: php artisan fuel:add "Your first task"');
    }
    ...
    protected function installGuidelines(): void
    {
        ...
        $guidelineConfig->usesFuel = $this->enableFuel;
