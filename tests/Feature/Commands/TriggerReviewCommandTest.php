...
it('triggers review for valid task', function (): void {
    ...
    $exitCode = Artisan::call('trigger:review', ['taskId' => $task->short_id]);
...
it('shows error for non-existent task', function (): void {
    $exitCode = Artisan::call('trigger:review', ['taskId' => 'f-nonexistent']);
    $output = Artisan::output();
...
it('uses agent from latest run when available', function (): void {
    ...
    Artisan::call('trigger:review', ['taskId' => $task->short_id]);
});
...
it('supports partial task ID matching', function (): void {
    ...
    Artisan::call('trigger:review', ['taskId' => $partialId]);
    $output = Artisan::output();
