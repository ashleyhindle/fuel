...
test('fromArray creates Task instance', function () {
    $data = [
        'id' => 1,
        'short_id' => 'f-abc123',
        'title' => 'Test Task',
...
test('isBlocked returns true when blocked_by is set', function () {
    $task = Task::fromArray(['blocked_by' => 'f-xyz789']);
    expect($task->isBlocked())->toBeTrue();
});

test('isBlocked returns false when blocked_by is null', function () {
    $task = Task::fromArray(['blocked_by' => null]);
    expect($task->isBlocked())->toBeFalse();
});

test('isBlocked returns false when blocked_by is empty string', function () {
...
test('getLabelsArray returns array of labels', function () {
    $task = Task::fromArray(['labels' => 'api,urgent,testing']);
    expect($task->getLabelsArray())->toBe(['api', 'urgent', 'testing']);
});

test('getLabelsArray returns empty array when labels is null', function () {
    $task = Task::fromArray(['labels' => null]);
    expect($task->getLabelsArray())->toBe([]);
});

test('getLabelsArray returns empty array when labels is empty string', function () {
    $task = Task::fromArray(['labels' => '']);
    expect($task->getLabelsArray())->toBe([]);
});

test('getLabelsArray handles labels with spaces', function () {
    $task = Task::fromArray(['labels' => 'api, urgent, testing']);
    expect($task->getLabelsArray())->toBe(['api', 'urgent', 'testing']);
});

test('getLabelsArray returns single label', function () {
    $task = Task::fromArray(['labels' => 'api']);
    expect($task->getLabelsArray())->toBe(['api']);
});

test('getBlockedByArray returns array of task IDs', function () {
    $task = Task::fromArray(['blocked_by' => 'f-abc123,f-xyz789,f-def456']);
    expect($task->getBlockedByArray())->toBe(['f-abc123', 'f-xyz789', 'f-def456']);
});

test('getBlockedByArray returns empty array when blocked_by is null', function () {
    $task = Task::fromArray(['blocked_by' => null]);
    expect($task->getBlockedByArray())->toBe([]);
});

test('getBlockedByArray returns empty array when blocked_by is empty string', function () {
    $task = Task::fromArray(['blocked_by' => '']);
    expect($task->getBlockedByArray())->toBe([]);
});

test('getBlockedByArray handles blocked_by with spaces', function () {
    $task = Task::fromArray(['blocked_by' => 'f-abc123, f-xyz789, f-def456']);
    expect($task->getBlockedByArray())->toBe(['f-abc123', 'f-xyz789', 'f-def456']);
});

test('getBlockedByArray returns single task ID', function () {
