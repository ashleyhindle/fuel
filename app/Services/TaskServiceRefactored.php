...
/**
 * Refactored TaskService using the repository pattern for standardized database access.
 * This can replace the original TaskService once testing is complete.
 */
class TaskServiceRefactored
{
    ...
    public function __construct(private readonly DatabaseService $db)
    {
        ...
        $this->epicRepository = new EpicRepository($db);
    }
    ...
    public function create(array $data): Task
    {
        ...
        // Insert the task using repository
    ...
    // ... Rest of the methods remain mostly the same, using repository patterns ...
    ...
    /**
     * Helper methods from original TaskService
     */
    ...
    private function rowToTask(array $row): array
    {
        ...
        if (isset($row['epic_id']) && $row['epic_id'] !== null) {
            $epicShortId = $this->epicRepository->resolveToShortId((int) $row['epic_id']);
            $task['epic_id'] = $epicShortId;
        } else {
            $task['epic_id'] = null;
        }
    ...
    // Additional methods that delegate to other services
