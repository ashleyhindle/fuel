...
This project uses **Fuel** for lightweight task tracking. Tasks are stored in `.fuel/tasks.jsonl`.
...
2. **File tasks for incomplete work** - Any unfinished work or follow-ups need tasks:
   ```bash
   ./fuel add "Remaining work description" --priority=2
   ```

3. **Verify task state** - Check nothing is left dangling:
   ```bash
   ./fuel list --status=open
   ```
...
```bash
./fuel add "Design API" --type=task
./fuel add "Implement API" --blocked-by=fuel-a7f3
```

When you close a blocking task, dependent tasks become ready automatically.
